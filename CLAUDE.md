...
`public/` を戻し、共有物はビルド前のコピーで配ること**。今このやり方なのは、
共有したいものが数点しか無いあいだは publicDir を向けるのが一番安いから。

`ogp.png`（1200x630）もここにある。両サイトが同じ絵を使う前提で、中身は favicon と
同じ `f.` マーク + `fushihara.net`。**`og:image` は絶対 URL でないとクローラが解決
できない**ので、本体は直書き、ブログは `Astro.site` から組み立てている（出力は
`/blog/ogp.png`）。配線が切れても画面には出ないので、本体は `e2e/favicon.spec.ts`、
ブログは `blog/e2e/blog.spec.ts` の「配信物」節が meta と実体を突き合わせている。
